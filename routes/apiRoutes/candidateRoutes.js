...
// since moving from server.js file we can change the following:
// app object should be changed to router
// remove /api from prefixes bc it's declared in server.js
// approx line 13 - app.use('/api', apiRoutes);
...
// get ALL candidates
router.get('/candidates', (req, res) => { // endpoint /api/candidates. Remember, the api in the URL signifies that this is an API endpoint
...
router.post('/candidate', ({ body }, res) => { // in callback function, use the object req.body to populate the candidate's data
